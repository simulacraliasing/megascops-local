; Basic Information
Name "Megascops-local OpenVINO Patch"
OutFile "Megascops-local-ov_patch.exe"
Unicode True
...
Section "Install DLL Patch"
  ...
  FileOpen $R1 "$INSTDIR\ov_patch_info.txt" w
  FileWrite $R1 "ov_version: ${PATCH_VERSION}$\r$\n"
  FileWrite $R1 "ov_updated_files:$\r$\n"
  
  ; Include all DLL files from the bin directory
  ; First, create a temporary directory to extract DLLs
  SetOutPath "$TEMP\megascops_ov_patch"
  
  ; Include all DLLs from the bin folder
  File /r "${SOURCE_DLL_FOLDER}\*.dll"
  
  ; Now process each DLL file
  FindFirst $0 $1 "$TEMP\megascops_ov_patch\*.dll"
  ${DoWhile} $1 != ""
    ${If} $LANGUAGE == ${LANG_SIMPCHINESE}
    ...
    ${EndIf}
    CopyFiles /SILENT "$TEMP\megascops_ov_patch\$1" "$INSTDIR\$1"
  ...
  RMDir /r "$TEMP\megascops_ov_patch"
