...
function getFFmpegInfo() {
    ...
    if (targetTriple === 'x86_64-pc-windows-msvc') {
        return {
            ...
            outputPath: path.join(binaryDir, 'ffmpeg-x86_64-pc-windows-msvc.zip'),
        ...
        return {
            ...
            outputPath: path.join(binaryDir, 'ffmpeg-x86_64-unknown-linux-gnu.tar.xz'),
            extractDir: binaryDir
        };
...
function getOrganizeInfo() {
    ...
    if (targetTriple === 'x86_64-pc-windows-msvc') {
        return {
            ...
            outputPath: path.join(binaryDir, 'organize-x86_64-pc-windows-msvc.exe'),
        };
    } else if (targetTriple === 'aarch64-apple-darwin') {
        return {
        ...
        return {
            ...
            outputPath: path.join(binaryDir, 'organize-x86_64-unknown-linux-gnu'),
        };
    } else if (targetTriple === 'aarch64-unknown-linux-gnu') {
        return {
            url: 'https://github.com/simulacraliasing/organize/releases/download/v0.1.0/organize-aarch64-unknown-linux-gnu',
            outputPath: path.join(binaryDir, 'organize-aarch64-unknown-linux-gnu'),
        }
    }
    else {
        throw new Error(`Unsupported target triple: ${targetTriple}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        const handleRedirect = (response) => {
            if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
                const newUrl = new URL(response.headers.location, fileUrl).toString();
    ...
    if (targetTriple === 'x86_64-pc-windows-msvc') {
        ...
        try {
            ...
            if (ffmpegExe) {
                ...
                copyFileSync(ffmpegExe, path.join(binaryDir, `ffmpeg-${targetTriple}.exe`));
...
async function downloadOrganize() {
    ...
    try {
        ...
        console.log('Organize has been successfully installed for your platform!');
    } catch (error) {
